...
class SearchView(View):
    def get(self, request, *args, **kwargs):
        try:
            ...
            if query:
                queryset1 = queryset1.filter(Q(title__icontains=query)|Q(price__icontains=query)).distinct()
                queryset = list(
                    sorted(
                        chain(queryset1),
            ...
            messages.info(self.request, "Search based on price and name")
...
class EcoDetail(DetailView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['category'] = Category.objects.all()
        context['iitem'] = Item.objects.filter()[0:3]
        return context
...
class CategoryDetail(DetailView, MultipleObjectMixin):
    ...
    def get_context_data(self, **kwargs):
        object_list = Item.objects.filter(category=self.object)
...
class CheckoutView(LoginRequiredMixin, TemplateView):
    model = Order
    template_name = 'prac/checkout.html'
    context_object_name = 'queryset'
...
# Add to cart Turn into CreateView
@login_required
def add_to_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            return redirect("eco:EcoDetail", slug=item.slug)
    else: 
...
def remove_from_cart(request, slug):
    ...
    if order_qs.exists():
        ...
        if order.items.filter(item__slug=item.slug).exists():
            ...
            return redirect("eco:EcoDetail", slug=slug)
    else:
...
class WishSummaryView(LoginRequiredMixin, View):
    def get(self, *args, **kwargs):
        try:
            ...
            messages.warning(self.request, "You do not have an Wishlist")
            return redirect("eco:EcoIndex")
...
def add_to_wish(request, slug):
    ...
    if created:
        ...
        messages.warning(request, "item was alredy in your wish")
    return redirect('eco:EcoDetail', slug=item.slug)
...
class ContactView(FormView):
    ...
    success_url = reverse_lazy('eco:contact') # we can have a specific page here..
...
class Accounts(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        form.save()
        try:      
            messages.warning(self.request, "Your account created successfuly")
            return redirect("eco:wish-summary")         
        except BadHeaderError:
            messages.warning(self.request, "Your email didn't successfuly")
            return HttpResponse('Invalid header found.')
        return super(Accounts, self).form_valid(form)
